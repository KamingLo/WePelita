...
<body class="min-h-screen">
    ...
    <nav class="lg:hidden fixed top-0 left-0 right-0 z-50 floating-nav">
        ...
        <div id="mobile-menu" class="absolute top-full left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 transform -translate-y-full opacity-0 transition-all duration-300 pointer-events-none">
            <div class="p-4 space-y-2">
                <a href="{{ route('orangtua.dashboard') }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('orangtua/dashboard*') || request()->is('profile*') ? 'active-nav' : '' }}">
                    <i class="fas fa-home w-5 text-center"></i>
                    <span class="sidebar-text">Halaman Utama</span>
                </a>
                <div class="my-4">
                    <h5 class="text-amber-400 font-semibold text-sm uppercase tracking-wide px-3 mb-2">Parent Tools</h5>
                    <div class="space-y-1">
                        <a href="{{ route('orangtua.jadwal') }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('orangtua/jadwal*') ? 'active-nav' : '' }}">
                            <i class="fas fa-calendar-days w-5 text-center"></i>
                            <span class="sidebar-text">Jadwal Kelas</span>
                        </a>
                        <a href="{{ route('orangtua.nilai') }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('orangtua/nilai*') ? 'active-nav' : '' }}">
                            <i class="fas fa-scroll w-5 text-center"></i>
                            <span class="sidebar-text">Nilai Anak</span>
                        </a>
                        <a href="{{ route('orangtua.pengumuman') }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('orangtua/pengumuman*') ? 'active-nav' : '' }}">
    ...
    <aside id="sidebar" class="hidden lg:flex fixed top-0 left-0 h-full sidebar-transition z-40 flex-col bg-gradient-to-b from-slate-800 to-slate-900 shadow-2xl border-r border-white/10">
        ...
        <nav class="flex-1 p-4 custom-scrollbar overflow-y-auto">
            <ul class="space-y-2">
                <li>
                    <a href="{{ route('orangtua.dashboard') }}" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('orangtua/dashboard*') || request()->is('profile*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                ...
                <li class="pt-4">
                    <h5 class="text-amber-400 font-semibold text-sm uppercase tracking-wide px-3 mb-3 sidebar-text">Parent Tools</h5>
                    <ul class="space-y-1">
                        <li>
                            <a href="{{ route('orangtua.jadwal') }}" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('orangtua/jadwal*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                        ...
                        <li>
                            <a href="{{ route('orangtua.nilai') }}" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('orangtua/nilai*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-scroll w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Nilai Anak</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('orangtua.pengumuman') }}" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('orangtua/pengumuman*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
        ...
        function initializeMobileMenu() {
            ...
            mobileMenuToggle.addEventListener('click', () => {
                ...
                if (isMenuOpen) {
                    ...
                    mobileMenu.classList.add('translate-y-0', Freed, 'pointer-events-auto');
