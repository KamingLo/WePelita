    ...
    <div id="main-content" class="px-4 md:px-8 py-8 flex flex-col gap-8 transition-all duration-400 ease-in-out">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <form method="GET" action="{{ route('admin.manajemenPost') }}" id="filterForm" class="flex items-center gap-3">
                    <label for="TipePost" class="font-medium text-gray-700 min-w-[90px]">Filter Tipe:</label>
                    <select name="TipePost" id="TipePost" class="w-full sm:w-48 p-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600" onchange="this.form.submit()">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                ...
                                <input class="w-full p-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600" type="text" id="judul" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul postingan" required>
                                @error('judul')
                                    <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="isi_trix" class="block text-sm font-medium text-gray-700 mb-1">Isi Konten</label>
                                <input id="isi_trix" type="hidden" name="isi_trix" value="{{ old('isi_trix') }}">
                                <trix-editor input="isi_trix" class="h-64 border border-gray-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600" placeholder="Tulis konten postingan Anda di sini..."></trix-editor>
                                @error('isi_trix')
                                    <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="flex justify-end">
                                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition">Posting</button>
                            </div>
                        </div>
                    </div>
                </form>
                        ...
                        @foreach($pengumumans as $pengumuman)
                            <div class="bg-white rounded-xl shadow-sm p-4 hover:shadow-md transition">
                                ...
                                <div class="flex gap-3">
                                    <a href="{{ route('admin.post.edit', ['id' => $pengumuman->postingan_id]) }}" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition text-center">Edit</a>
                                    <form action="{{ route('admin.post.hapus', ['id' => $pengumuman->postingan_id]) }}" method="POST" class="delete-post-form flex-1">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @if(isset($blogs) && $blogs->isNotEmpty())
                        @foreach($blogs as $blog)
                            <div class="bg-white rounded-xl shadow-sm p-4 hover:shadow-md transition">
                                ...
                                <div class="flex gap-3">
                                    <a href="{{ route('admin.post.edit', ['id' => $blog->postingan_id]) }}" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition text-center">Edit</a>
                                    <form action="{{ route('admin.post.hapus', ['id' => $blog->postingan_id]) }}" method="POST" class="delete-post-form flex-1">
                                        @csrf
                                        @method('DELETE')
