...
<head>
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 1023px) { /* Mobile: <1024px */
            #main-content {
                padding-bottom: 20rem; /* Tambahkan ruang bawah untuk mencegah tabrakan dengan menu mobile */
            }
        }
        
        @media (min-width: 1024px) { /* lg: di Tailwind */
            #main-content {
                margin-left: 18rem; /* lg:ml-72 */
            }
        ...
        /* Warna teks global di dark mode */
        html.dark h1, html.dark h4, html.dark h5, html.dark h6 {
            color: #ffffff; /* text-white untuk judul di dark mode */
        }

        html.dark #main-content {
            color: #000000; /* text-black untuk konten utama di dark mode */
        }

        html.light #main-content {
            color: #374151; /* text-gray-700 untuk light mode */
        }

        /* Sembunyikan logo di sidebar desktop saat minimized */
        .sidebar-minimized .sidebar-logo {
            display: none;
        }
    </style>
    <script>
        // Terapkan status theme dan sidebar dari localStorage segera
...
<body class="min-h-screen">
    ...
    <nav class="lg:hidden fixed top-0 left-0 right-0 z-50 floating-nav">
        ...
        <div id="mobile-menu" class="absolute top-full left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 transform -translate-y-full opacity-0 transition-all duration-300 pointer-events-none">
            <div class="p-4 space-y-2">
                <a href="/admin/dashboard" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/dashboard*') || request()->is('profile*') ? 'active-nav' : '' }}">
                    <i class="fas fa-home w-5 text-center"></i>
                    <span class="sidebar-text">Halaman Utama</span>
                </a>
                
                <div class="my-4">
                    <h5 class="text-amber-400 font-semibold text-sm uppercase tracking-wide px-3 mb-2">Admin Tools</h5>
                    <div class="space-y-1">
                        <a href="/admin/TambahPelajaran" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/TambahPelajaran*') || request()->is('admin/pelajaran*') ? 'active-nav' : '' }}">
                            <i class="fas fa-book w-5 text-center"></i>
                            <span class="sidebar-text">Tambah Pelajaran</span>
                        </a>
                        <a href="/admin/TambahJadwal" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/TambahJadwal*') || request()->is('admin/jadwal/edit/*') ? 'active-nav' : '' }}">
                            <i class="fas fa-calendar-days w-5 text-center"></i>
                            <span class="sidebar-text">Tambahkan Jadwal</span>
                        </a>
                        <a href="/admin/manajemenKelas" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/manajemenKelas*') ? 'active-nav' : '' }}">
                            <i class="fas fa-pen w-5 text-center"></i>
                            <span class="sidebar-text">Manajemen Kelas</span>
                        </a>
                        <a href="/admin/ManajemenUser" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is(['admin/ManajemenUser*', 'admin/user*']) ? 'active-nav' : '' }}">
                            <i class="fas fa-users w-5 text-center"></i>
                            <span class="sidebar-text">Manajemen User</span>
                        </a>
                        <a href="/admin/manajemenPost" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/manajemenPost*') ? 'active-nav' : '' }}">
                            <i class="fas fa-bullhorn w-5 text-center"></i>
                            <span class="sidebar-text">Manajemen Postingan</span>
                        </a>
                        <a href="/admin/NilaiSiswa" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 text-white nav-item {{ request()->is('admin/NilaiSiswa*') ? 'active-nav' : '' }}">
    ...
    <aside id="sidebar" class="hidden lg:flex fixed top-0 left-0 h-full sidebar-transition z-40 flex-col bg-gradient-to-b from-slate-800 to-slate-900 shadow-2xl border-r border-white/10">
        ...
        <nav class="flex-1 p-4 custom-scrollbar overflow-y-auto">
            <ul class="space-y-2">
                <li>
                    <a href="/admin/dashboard" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/dashboard*') || request()->is('profile*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                ...
                <li class="pt-4">
                    <h5 class="text-amber-400 font-semibold text-sm uppercase tracking-wide px-3 mb-3 sidebar-text">Admin Tools</h5>
                    <ul class="space-y-1">
                        <li>
                            <a href="/admin/TambahPelajaran" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/TambahPelajaran*') || request()->is('admin/pelajaran*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-book w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Tambah Pelajaran</span>
                            </a>
                        </li>
                        <li>
                            <a href="/admin/TambahJadwal" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/TambahJadwal*') || request()->is('admin/jadwal/edit/*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-calendar-days w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Tambahkan Jadwal</span>
                            </a>
                        </li>
                        <li>
                            <a href="/admin/manajemenKelas" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/manajemenKelas*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-pen w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Manajemen Kelas</span>
                            </a>
                        </li>
                        <li>
                            <a href="/admin/ManajemenUser" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is(['admin/ManajemenUser*', 'admin/user*']) ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-users w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Manajemen User</span>
                            </a>
                        </li>
                        <li>
                            <a href="/admin/manajemenPost" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/manajemenPost*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-bullhorn w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Manajemen Postingan</span>
                            </a>
                        </li>
                        <li>
                            <a href="/admin/NilaiSiswa" class="flex items-center space-x-3 p-3 rounded-xl transition-all duration-200 nav-item group {{ request()->is('admin/NilaiSiswa*') ? 'active-nav' : 'hover:bg-white/10 text-white' }}">
                                <i class="fas fa-scroll w-5 text-center group-hover:scale-110 transition-transform duration-200"></i>
                                <span class="sidebar-text font-medium">Nilai Siswa</span>
                            </a>
                        </li>
                    </ul>
                </li>
    ...
    <script>
        ...
        function initializeThemeToggle() {
            ...
            // Inisialisasi tema berdasarkan localStorage
