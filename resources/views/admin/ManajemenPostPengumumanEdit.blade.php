@include('admin.partials.header', ['NamaPage' => 'Edit Pengumuman'])
...
<div class="ContainerPostManagement">
    ...
    <div class="edit-container">
        <div class="form-container">
            <div class="form-col">
                <form action="{{ route('edit.pengumuman', ['id' => $pengumuman->pengumuman_id]) }}" method="POST" enctype="multipart/form-data" id="editForm">
                    @csrf
                    <div class="mb-3">
                        <label for="judul" class="form-label">Judul</label>
                        <input type="text" class="form-control" id="judul" name="judul" required
                            value="{{ old('judul', $pengumuman->judul_pengumuman) }}" oninput="updatePreview()">
                    </div>

                    <div class="mb-3">
                        <label for="isi" class="form-label">Isi</label>
                        <textarea class="form-control" id="isi" name="isi" rows="4" required oninput="updatePreview()">{{ old('isi', $pengumuman->isi_pengumuman) }}</textarea>
                    </div>
            ...
            <div class="preview-container" id="previewContainer">
                ...
                <div class="preview-title" id="previewTitle">
                    {{ old('judul', $pengumuman->judul_pengumuman) }}
                </div>
                <div class="preview-content" id="previewContent">
                    {{ old('isi', $pengumuman->isi_pengumuman) }}
                </div>
...
<script>
    ...
    function previewImage(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            const currentImage = '{{ $pengumuman->lampiran ? asset("storage/" . $pengumuman->lampiran) : "" }}';
            if (!currentImage) {
