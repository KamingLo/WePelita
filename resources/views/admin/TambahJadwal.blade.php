    ...
    <div id="main-content" class="px-4 md:px-8 py-8 flex flex-col gap-8 transition-all duration-400 ease-in-out">
        ...
        <div class="flex flex-col md:flex-row md:gap-6 flex-1">
            <div class="bg-white p-4 rounded-lg shadow-md w-full md:w-1/3 flex flex-col mb-6 md:mb-0 md:h-[calc(100vh-10rem)]">
                ...
                <form action="{{ route('TambahJadwal.store') }}" method="POST" class="space-y-4 flex-1 overflow-y-auto">
                    @csrf
                                ...
                                <tr class="{{ $loop->odd ? 'bg-gray-100' : 'bg-white' }}">
                                    ...
                                    <td class="p-2 border-b text-center whitespace-nowrap">
                                        <div class="flex flex-col items-center justify-center gap-2 mt-2">
                                            <a href="jadwal/edit/{{ $jadwal->jadwal_id }}" class="bg-blue-600 text-white px-3 py-1.5 h-8 rounded text-xs hover:bg-blue-700 w-[60px] text-center">Edit</a>
                                            <form action="{{ route('jadwal.destroy', $jadwal->jadwal_id) }}" method="POST" class="inline-block w-full md:w-auto">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bg-red-600 text-white px-3 py-1.5 h-8 rounded text-xs hover:bg-red-700 w-full text-center">
                                                    Hapus
                                                </button>
                                            </form>
                                        </div>
