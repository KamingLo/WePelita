                ...
                <form id="postForm" action="{{ route('guru.post.update', ['id' => $postingan->postingan_id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-x-6 gap-y-4 md:gap-y-6">
                        <div class="flex flex-col max-w-full md:max-w-[420px]">
                            ...
                            <div class="mb-4 md:mb-5" id="tujuanContainer" style="display: {{ old('tipe', $postingan->tipe) == 'pengumuman' ? 'block' : 'none' }};">
                                ...
                                <div class="relative">
                                    ...
                                    <div class="flex items-center justify-between w-full p-2 md:p-3 border border-gray-300 rounded-md text-sm bg-white text-gray-600">
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button" class="bg-red-500 text-white px-3 py-1.5 rounded text-xs hover:bg-red-600 transition h-10 flex items-center justify-center {{ $postingan->lampiran ? '' : 'hidden' }}" id="deleteButton">Hapus</button>
                                        </div>
                                    </div>
                                    @error('lampiran')
                                        <span class="text-red-500 text-xs md:text-sm mt-1">{{ $message }}</span>
                            ...
                            <div class="mt-3 md:mt-4 p-4 md:p-5 border-2 border-dashed border-gray-300 rounded-lg md:rounded-xl bg-gray-50 text-center h-40 md:h-48 overflow-hidden transition {{ $postingan->lampiran ? 'border-blue-500 bg-white' : '' }}" id="previewContainer">
                                @if($postingan->lampiran)
                                    <img id="previewImage" class="max-w-full max-h-32 md:max-h-40 rounded-md md:rounded-lg shadow-md object-contain" src="{{ asset('storage/' . $postingan->lampiran) }}" alt="Preview" style="display: block;">
                                    <div class="previewText text-gray-600 text-xs md:text-sm mt-2 hidden">Preview foto akan muncul di sini</div>
                                @else
                        ...
                        <div class="flex flex-col relative pt-0 md:pt-14">
                            <div class="flex flex-col md:flex-row gap-4 md:gap-5 items-start md:items-center mb-4 md:mb-0 md:justify-end md:absolute md:top-0 md:right-0">
                                ...
                                <a href="{{ route('guru.ManajemenPost', ['TipePost' => $postingan->tipe]) }}" class="bg-red-500 text-white px-4 md:px-6 py-2 md:py-3 rounded-md font-medium hover:bg-red-600 hover:scale-105 transition shadow-md w-full md:w-32 h-12 flex items-center justify-center">Cancel</a>
            ...
            lampiranInput.addEventListener('change', function() {
                if (this.files && this.files[0]) {
                    ...
                    reader.onload = function(e) {
                        ...
                        previewImage.style.display = 'block'; // Ensure image is displayed
