    ...
    <div class="ml-0 md:ml-64 p-4 md:p-8 flex flex-wrap gap-4 md:gap-6 min-h-[82vh] md:min-h-[calc(100vh-100px)] pb-10 md:pb-20">
        ...
        <div class="flex flex-col md:flex-row gap-4 md:gap-5 w-full -mt-4">
            <div class="w-full min-w-[380px] md:w-96 bg-white py-3 px-4 md:py-3 md:px-5 rounded-lg md:rounded-xl shadow-md md:shadow-lg flex items-center">
                <form method="GET" action="{{ route('guru.ManajemenPost') }}" id="filterForm" class="flex items-center flex-wrap md:flex-nowrap gap-x-3 gap-y-2">
        ...
        <div class="w-full bg-white p-4 md:p-6 rounded-lg md:rounded-xl shadow-md md:shadow-lg mb-4 md:mb-6 h-auto overflow-hidden">
            <div class="{{ request('TipePost') == '' ? 'block' : 'hidden' }}" id="tambahPost">
                ...
                <form id="postForm" action="{{ route('guru.post.tambah') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-x-6 gap-y-4 md:gap-y-6"> {{-- Adjusted gap-x for spacing, added gap-y --}}
                        ...
                        <div class="flex flex-col relative pt-0 md:pt-14"> {{-- Added padding top for larger screens --}}
                            <div class="flex flex-col md:flex-row gap-4 md:gap-5 items-start md:items-center mb-4 md:mb-0 md:justify-end md:absolute md:top-0 md:right-0"> {{-- Absolute positioning for submit button on larger screens --}}
                            ...
                            <div class="mt-4 md:mt-0 mb-4 md:mb-5"> {{-- Adjusted margin-top --}}
                            ...
                            <div class="max-w-full">
                                <div class="mb-4 md:mb-5">
                                    <label for="isi" class="block mb-1 md:mb-2 font-medium text-gray-800 text-sm">Isi Konten</label>
                                    <input id="isi" type="hidden" name="isi" value="{{ old('isi') }}">
                                    <trix-editor input="isi" class="h-64 md:h-72 min-h-40 md:min-h-48 max-h-64 md:max-h-72 overflow-y-auto border border-gray-300 rounded-md p-3 md:p-4 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="Tulis konten postingan Anda di sini..."></trix-editor>
                                    @error('isi')
                                        <span class="text-red-500 text-xs md:text-sm mt-1">{{ $message }}</span>
                                    @enderror
                                </div>
                    ...
                    @if(isset($pengumumans) && $pengumumans->isNotEmpty())
                        @foreach($pengumumans as $pengumuman)
                            <div class="bg-white border border-gray-200 rounded-lg md:rounded-xl p-4 md:p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition flex flex-col md:flex-row gap-4 md:gap-5 items-start md:items-stretch min-h-[192px] md:min-h-48">
                                ...
                                {{-- PERBAIKAN DI SINI: Hapus justify-between dari flex-col --}}
                                <div class="flex-1 flex flex-col gap-y-2 md:gap-y-3">
                                    ...
                                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-3 mt-auto pt-2">
                                        ...
                                        <div class="flex gap-2 w-full md:w-auto">
                                            <a href="{{ route('guru.post.edit', ['id' => $pengumuman->postingan_id]) }}" class="max-h-[37px] bg-blue-500 text-white px-3 md:px-4 py-1.5 md:py-2 rounded-md text-xs md:text-sm font-medium hover:bg-blue-600 hover:shadow-md transition w-full md:w-auto flex items-center justify-center h-8 md:w-auto">Edit</a>
                                            <form action="{{ route('guru.post.hapus', ['id' => $pengumuman->postingan_id]) }}" method="POST" class="delete-post-form w-full md:w-auto">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg md:rounded-xl p-4 md:p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition flex flex-col md:flex-row gap-4 md:gap-5 items-start md:items-stretch min-h-[192px] md:min-h-48">
                                ...
                                <div class="flex-1 flex flex-col gap-y-2 md:gap-y-3">
                                    ...
                                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-3 mt-auto pt-2">
                                        ...
                                        <div class="flex gap-2 w-full md:w-auto">
                                            <a href="{{ route('guru.post.edit', ['id' => $blog->postingan_id]) }}" class="bg-blue-500 text-white px-3 md:px-4 py-1.5 md:py-2 rounded-md text-xs md:text-sm font-medium hover:bg-blue-600 hover:shadow-md transition w-full md:w-auto flex items-center justify-center h-8 md:w-auto">Edit</a>
                                            <form action="{{ route('guru.post.hapus', ['id' => $blog->postingan_id]) }}" method="POST" class="delete-post-form w-full md:w-auto">
                                                @csrf
                                                @method('DELETE')
                ...
                if (checkedLabel) {
                    ...
                    switchLabels.forEach(label => {
                        if (label === checkedLabel) {
                            label.classList.remove('text-gray-700'); // Remove inactive color
                            label.classList.add('text-white'); // Add active color (white, as per your request)
                        } else {
                            label.classList.remove('text-white'); // Remove active color
                            label.classList.add('text-gray-700'); // Add inactive color (gray-700)
                        }
                    });
                }
            }
